...
public class HubShop : MonoBehaviour
{
    ...
    void RefreshUI()
    {
        ...
        UpdateUpgradeUI(hpProgressBar, hpLevelText, hpCostText, GameManager.instance.healthLevel);
        UpdateUpgradeUI(oxygenProgressBar, oxygenLevelText, oxygenCostText, GameManager.instance.oxygenLevel);
        UpdateUpgradeUI(spdProgressBar, spdLevelText, spdCostText, GameManager.instance.speedLevel);
    }

    void UpdateUpgradeUI(Image bar, TextMeshProUGUI levelText, TextMeshProUGUI costText, int currentLevel)
    {
        if (bar != null) bar.fillAmount = (float)currentLevel / maxLevel;
        if (levelText != null) levelText.text = "Level " + currentLevel + " / " + maxLevel;
